...
\begin{frame}{Results: Quantitative}
  ...
  \end{center}
  \vspace{0.5em}
  We can notice that the gap is not marginal: U-Net leads DeepLabV3 by \textbf{0.24 IoU points}. \\[0.4em]
  \textbf{Resolution Paradox}: Training at $512\times512$ visibly preserves thinner structures better than $256\times256$, but yields a slightly \textit{lower} IoU (0.565). Why? Downsampling heavily blurs and "engorges" roads, making them easier to overlap artificially, whereas high resolution demands strict pixel-perfect matching.
\end{frame}
...
\begin{frame}{Results: Visual Comparison}
  \begin{columns}[T]
    ...
    \centering
    \textbf{U-Net}
    \includegraphics[width=\linewidth]{unet/unet_predictions_valid.png}
    \small Sharp, connected outlines; thin rural tracks recovered.

    \column{0.5\textwidth}
    \centering
    \textbf{DeepLabV3}
    \includegraphics[width=\linewidth]{deeplabv3/deeplabv3_predictions_valid.png}
...
\begin{frame}{Paths Forward \& Limitations}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Train at $512\times512$ --- thinnest roads likely lost at current resolution
    ...
    \begin{itemize}
      ...
      \item $256\times256$ resizing discards spatial detail
    \end{itemize}
  \end{columns}
\end{frame}
